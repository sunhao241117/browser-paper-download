...
**批量下载最佳实践（从515篇项目总结）：**
1. **下载优先级三级流程：**
   - **第一级（最高优先级）：PMC 开放获取文献** — 最稳定、最快、成功率最高（~95%）。先处理所有有 PMCID 的文章。
   - **第二级：出版商机构访问文献** — PMC 处理完后，按出版商分组处理（Elsevier → Wiley → Springer → 其他），利用机构订阅权限。
   - **第三级（最后处理）：Sci-Hub fallback** — 前两级都失败的文献，且发表年份 ≤ 2021 年的，尝试通过 Sci-Hub 获取。
2. **已知无权限期刊直接跳过**，不要浪费时间重复尝试（但 2021 年之前的仍可走 Sci-Hub 路线）
...
7. **所有权限提示自动批准**，不询问人工
8. **2021年及之前的文献优先尝试 Sci-Hub** — 对于通过出版商路线找不到 PDF 的文献，如果发表年份 ≤ 2021 年，可以尝试通过 Sci-Hub 获取（Scholarscope 插件自动解析 Sci-Hub 镜像，或直接访问 www.sci-hub.st/{doi}）。2022 年及之后的文献 Sci-Hub 通常未收录，无需尝试。
